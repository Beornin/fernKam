...
│                        saved_searches, backup, logs, debug
├── services/
...
maintenance, then any tasks left "running" by a previous process are marked cancelled.
InsightFace warms up in the background. Every step is idempotent and logs instead of crashing.
...
and matches on id *and* path so a row moved mid-scan survives. Photo trash and dedup
...
`sm`/`md`/`lg`/`xl`/`xxl`), written atomically, generated on demand if missing. Face crops
(200 px) are stored in `faces.crop_data`.

**XMP write-back** (`metadata_sync.write_metadata_batch`) runs one exiftool process per batch.
After writing, call `mark_files_synced()`. It records each file's new mtime, otherwise the next
scan re-reads the file and overwrites newer in-app edits with the file's values.
